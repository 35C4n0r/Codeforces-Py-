...
def manipulated_seive(N):
    ...
    for i in range(2, N):
        ...
        # If isPrime[kk] == True then kk is
        # prime number
        if isprime[i] == True:
            # put kk into prime[] vector
            prime.append(i)

            # lis prime number is its own smallest
            # prime factor
            SPF[i] = i

        # Remove all multiples of kk*prime[j]
        # which are not prime by making is
        # Prime[kk * prime[j]] = false and put
        # smallest prime factor of kk*Prime[j]
        # as prime[j] [ for exp :let kk = 5 , j = 0 ,
        # prime[j] = 2 [ kk*prime[j] = 10 ]
        ...
        while (j < len(prime) and
            ...
            # put smallest prime factor of kk*prime[j]
...
if __name__ == "__main__":
    ...
    N = 100  # Must be less than MAX_SIZE
